...
- **整个时段五个事件全是 0 就不判断**（`window_empty`）。2026-09-24 GA4 处理积压超过 3 小时，16:30–17:00 那个时段浏览 0（平时 180）、加购 0（平时 12.5），`add_to_cart_zero` 响了；而同一时段合成探测过了三次、页面 GA4 埋点在正常发 `page_view`、17:24 还成交了一单，19 分钟后重查同一段已经有 30 次浏览——数据只是还没到。
- 为什么不是继续加大滞后：积压有多长是 Google 说了算，追它只会让第4层越来越慢直到没用。**「五个事件全是 0」是唯一不可能由店铺故障造成的形状**——店坏了人照样会落地，浏览不会归零，其余规则照常工作。这条守卫只在**第1层说站点正常**时生效；第1层也不正常时，那些 0 可能是真的，规则仍然可以说话。
- 连续 `empty_window_notify_after`（6 个，约 3 小时）个空时段会发一条**静默的数据质量**提醒，措辞明写「这不是店坏了」。埋点被主题或 app 拿掉也长这样，所以不能永远不说；但它不是业务告警，不该把人训练成忽略下一次真的。
...
- **探测的结论会进到第4层的业务告警里**。第4层判断的是两小时前那个时段，所以问的不是「现在能不能买」而是「那时能不能买」：探测留 12 小时滚动记录，告警里那一行读的是覆盖该时段（前后各放宽 20 分钟，因为探测 20 分钟一次、窗口 30 分钟，严格落在窗口内的常常只有一次）的运行结果。
