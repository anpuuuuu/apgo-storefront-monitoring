...
- Checkout：当前 ATC ≥10、同期 Checkout ≥2、连续两个窗口 Checkout=0，且 **最近 60 分钟 Shopify 没有订单**（订单心跳推送是「结账能不能用」的第一方真相；有订单就不是结账坏了，最多是 GA4 漏记，交给 `purchase_tracking_gap`）。2026-09-15 凌晨两次误报（ATC 5 / 7、结账 0）促成了这两个条件；`begin_checkout_drop` 同样受订单条件约束。
