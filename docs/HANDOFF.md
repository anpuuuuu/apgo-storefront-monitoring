...
**Latest objective:** full central Live ownership and manual/post-retirement acceptance are verified. Live Daily passed 15/15; automatic retirement Post-deploy passed 13/13 on the exact retired Theme commit, with no retries/failures/missing results and no Daily-heartbeat overwrite. Unrestricted GA4 diagnostic, Validate, Primary, Confirm, Realtime, self-health, protected-main Worker deployment and NEXT-only self-test all passed. Old WIF/provider access and all Theme repository Actions Secrets are retired. The old Cloudflare token is positively identified, but provider-side revocation remains deferred because absence of dormant/shared copies cannot be proven from audit history. Actual post-cutover scheduled execution remains separately pending; first Daily is due September 4 at 09:37 MYT. See [FINAL-CUTOVER.md](FINAL-CUTOVER.md) for evidence and safe credential/rollback gates.
...
- Central owns all monitoring workflows and the existing Error Worker deployment. Theme main `65546a24ba91a607e50e37013931db884b39a1ab` contains no tracked monitoring directory or workflows, only a central pointer and unchanged frontend Layer 3 integration. The final Theme `CF_API_TOKEN` copy was deleted at approximately 21:38 MYT; API confirms zero repository Actions Secrets and zero workflows. Old heartbeat value, monitoring variables, WIF provider access and principal are retired. Cloudflare token `編輯 Cloudflare Workers` is identified through exact deploy/audit version correlation; all 1,666 observed operations in the reviewed account/time window target monitoring resources only, but the token itself is retained until sharing is definitively resolved. Central token is unchanged.
- At 21:42 MYT all four namespaced heartbeats are healthy, without legacy fallback. No post-full-Live `event=schedule` acceptance was yet observed. A bounded hourly read-only follow-up, **中央监控首次排程验收** (ID `automation`), now verifies actual schedules; it is not the deleted Shadow automation and does not dispatch test jobs or alter production. Pause it after completion or by September 5 at 10:00 MYT, reporting only meaningful changes or missing acceptance. Do not treat the historical 'no Codex review automation' statements below as its current status.
...
- **Scheduling finding (2026-09-08)**: GitHub delivered only ~18% of the 30-minute Layer 4 crons between 2026-08-30 and 2026-09-08 (8–10 `event=schedule` runs per day of 50 expected; median gap 158 min, p90 295 min) and the daily crons 4–6 hours late (primary 16:23–17:25 MYT, confirm 18:56–21:02 MYT on 09-04..09-07). The hourly self-health cron ran 5–8 times per day. The daily `site-health-v2.yml` cron and the Cloudflare `*/5` Worker cron were reliable. Every Layer 4 run that did execute succeeded; the repeated `layer4 heartbeat is delayed/stale` alerts were correct. PR [#44](https://github.com/anpuuuuu/apgo-storefront-monitoring/pull/44) moved scheduling to the Dispatcher Cron. **Live since 2026-09-09 08:54:49 UTC (16:54 MYT)** after #47 (dry-run), #48 (service-binding fix for the cross-Worker fetch refusal) and #49 (live); Dispatcher version `a5921cbc`. First two hours: layer4 heartbeat gaps 4/15/29/23/31 min, both daily stages completed, GitHub's late daily cron correctly became a `daily-skip`. Evidence table in [FINAL-CUTOVER.md](FINAL-CUTOVER.md).
...
- PR [#44](https://github.com/anpuuuuu/apgo-storefront-monitoring/pull/44) makes the Dispatcher Worker the Layer 4 scheduler (`workers/dispatcher/scheduler.mjs`, `*/5` cron): it dispatches `realtime` when the `/health` layer4 heartbeat is ≥28 minutes old, `daily-primary`/`daily-confirm` after 04:25/06:55 UTC (KV markers per UTC day), and self-health when layer3 is >90 minutes old, with KV locks, a 15-minute recent-run gate and a 60-minute failure back-off. `SCHEDULER_ENABLED` (shipped `false`) and `SCHEDULER_DRY_RUN` gate the rollout; `monitor-alerts.yml` now honours `MONITOR_LAYER4_PAUSED` for `trigger=scheduler`; the GitHub watchdog threshold dropped from 100 to 45 minutes. `ga4-anomaly` requires adjacent samples for "two consecutive windows" (>45 min restarts, <15 min is a duplicate) and logs coverage; the daily report flags `REALTIME_COVERAGE_LOW` below 80%. The daily funnel skips a repeated stage for the same `targetDate` within 12 hours (`daily-skip` heartbeat) and persists `ga4:daily:confirmed:<date>`. Worker/helper tests 67/67. Rollout completed 2026-09-09 via #47 → #48 → #49 (see the FINAL-CUTOVER evidence table); rollback is `SCHEDULER_ENABLED=false` and redeploy. Pending follow-ups: `/health` `layer2` was 227 minutes old at 10:48Z on 2026-09-09 because GitHub's daily `site-health-v2.yml` cron was also late that day — consider adding a Layer 2 deadline to `planDispatches`; the read-only D1 check of `alert_log` could not be run from the local OAuth token (code 7403), so Telegram duplicate-suppression was verified only through run outputs (`recorded: false` on every rule).
