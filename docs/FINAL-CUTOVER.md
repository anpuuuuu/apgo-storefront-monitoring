...
### Dispatcher Cron scheduler rollout (2026-09-09)

GitHub delivered ~18% of the 30-minute Layer 4 crons (see HANDOFF). PR [#44](https://github.com/anpuuuuu/apgo-storefront-monitoring/pull/44) moved scheduling to the Dispatcher Worker; the rollout followed the disabled → dry-run → live sequence. The first dry-run (#47) surfaced `health_unreadable` on every tick because a Worker cannot `fetch()` another workers.dev hostname on the same account (Cloudflare error 1042); #48 replaced the fetch with a `MONITOR` service binding.

| Check | Run / version | Evidence |
| --- | --- | --- |
| Dispatcher deploy, scheduler disabled | [34234535574](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34234535574) | Version `566ca0cd`; `schedule: */5 * * * *` registered; `SCHEDULER_ENABLED=false` |
| Dry-run enabled (#47) | [34319156899](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34319156899) | Version `6ce2ac56`; every tick logged `skipped: health_unreadable` — cross-Worker fetch refused |
| Service-binding fix (#48) | [34330998268](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34330998268) | Version `82131ab2`; tick 08:50:31Z (dry) → `decisions=daily-primary,layer3`, `skipped=daily-confirm:primary_not_dispatched layer4:workflow_claimed_this_tick` against a 205-minute-old layer4 heartbeat |
| Live (#49) | [34331673973](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34331673973) | Version `a5921cbc`, `SCHEDULER_DRY_RUN=false` at 08:54:49Z |
| First live tick 08:55:31Z | [34331812141](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34331812141), [34331814731](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34331814731) | `daily-primary` computed target `20260908` at `08:55:54Z` (0 anomalies); self-health closed a 244-minute Layer 3 gap |
| Daily idempotency | [34332190623](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34332190623) | GitHub's own `17 4 * * *` cron arrived at 08:59:48Z (4 h 42 min late, `event=schedule`) and exited `already_recorded` with a `daily-skip` heartbeat; the 09:00/09:05/09:10 ticks held `daily-confirm` on `recent_run:completed` |
| Daily confirm | [34333696011](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34333696011) | Dispatched at 09:15:31Z once the 15-minute gate expired; `stage: confirm`, `primaryGeneratedAt 08:55:54Z`, 0 persistent anomalies |
| Realtime cadence | [34336509473](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/runs/34336509473), [10:40:35Z dispatch](https://github.com/anpuuuuu/apgo-storefront-monitoring/actions/workflows/monitor-alerts.yml) | Worker dispatches at 09:45 and 10:40 (heartbeat age 30 min); GitHub's `49` cron landed 10:08:42Z and the 10:10 tick correctly saw `layer4:fresh(2m)`. Rules recorded `gapMinutes 262.5, coverageGap: true` for the pre-live gap |
| `/health` observation 08:54–10:48Z | polled every 150 s | layer4 heartbeats at 08:55:54, 09:00:09, 09:15:53, 09:45:53, 10:09:01, 10:40:55; gaps 4 / 15 / 29 / 23 / 31 min, all ≤ 40; 22 ticks, 0 `health_unreadable`, 0 scheduler failures |
